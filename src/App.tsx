...
function AppContent() {
  ...
  const handleAdd = useCallback(
    (payload: TaskInput) => {
      addTask(payload);
      pushActivity(createActivity('add', `Added: ${payload.title}`));
    },
    [addTask, createActivity, pushActivity]
  );
  ...
  const handleUpdate = useCallback(
    ...
    [updateTask, createActivity, pushActivity]
  );

  const handleDelete = useCallback(
    (id: string) => {
      deleteTask(id);
      pushActivity(createActivity('delete', `Deleted task ${id}`));
    },
    [deleteTask, createActivity, pushActivity]
  );

  const handleUndo = useCallback(() => {
    undoDelete();
    pushActivity(createActivity('undo', 'Undo delete'));
  }, [undoDelete, createActivity, pushActivity]);
